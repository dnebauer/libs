...
sub _discover_dirs ($self) {    ## no critic (RequireInterpolationOfMetachars)
  ...
  $self->write_log("Configuration directory is: $conf_dir", $INFO);
  if (not -d $conf_dir) {
  ...
  my @conf_fps = map { File::Spec->catfile($conf_dir, $_) } @conf_files;
...
sub _main_loop ($self) {    ## no critic (RequireInterpolationOfMetachars)
  ...
  my ($loop, $delay) = (0, $self->check_interval);
  while ($TRUE) {
...
C<config_dir> attribute set to true will print the configuration directory path
...
systems), but the definitive value is obtained with the C<config_dir> attribute.
