...
sub write_nvim_loader ($self, $nvim_loader, $master) {
  ...
  push @data, q{-- into a table.};
  push @data, q{};
  push @data, $generator;
  push @data, q{};
  push @data, q{-- -------------------------------------------};
  push @data, q{};
  push @data, q{---@diagnostic disable:unused-local};              ## no critic (RequireInterpolationOfMetachars)
  push @data, q{};
  push @data, qq[local $VIM_VARIABLE = {}];
  push @data, q{};

  # • main variable output here
  foreach my $func_name ($self->_function_names) {
    ...
    push @data, sprintf '%s["%s"] = %s', $VIM_VARIABLE, $func_name,
        $function->write_nvim_function_loader($func_name);
  }

  # • load variable into vim variable
  push @data, q{};
  my $assign = sprintf q{vim.api.nvim_buf_set_var(0, "%s", %s)},
      $VIM_VARIABLE, $VIM_VARIABLE;
  push @data, $assign;
...
This documentation is for Dn::CommonBash version 5.40.
