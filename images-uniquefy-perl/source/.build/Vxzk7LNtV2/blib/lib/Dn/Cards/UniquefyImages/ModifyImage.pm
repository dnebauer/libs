package Dn::Cards::UniquefyImages::ModifyImage;
...
method _build_width () {
    croak 'Image is not created' if not $self->_image;
...
sub _build__image () {
    croak 'No image filepath provided' if not $self->filepath;
...
method modify_pixel () {
    ...
    if ( ( $rgb_value <= 0 ) or ( $rgb_value > 255 ) ) {
        croak "Invalid RGB value '$rgb_value'";
    }
    ...
    $self->image_pixel_color( $image, $self->pixel_coords(qw(x y)),
        [@color] );
...
method pixel_rgb_component_index ($index = undef) {
    ...
    if ( defined $index ) {                         # setter
        ...
        my @match = grep { $index =~ /^$_/ } qw(0 1 2);
        croak if not @match;
        ...
        croak 'RGB component index not set'
...
method pixel_coords (@coords) {
    ...
    if (@coords) {              # setter
        ...
        # need two coords
        my $count = scalar @coords;
        croak "Need 2 arguments (got $count)" if $count != 2;

        # need integer coords
        for my $coord (@coords) {
            croak "Non-integer coordinate '$coord'"
                if not $self->int_pos($coord);
        }
        ...
        croak 'Pixel coordinates not set' if not $self->_has_pixel_coords;
...
# _has_pixel_coords()                                                  {{{1
#
# does:   whether both x and y pixel coords are available
# params: nil
# prints: nil
# return: scalar boolean
method _has_pixel_coords () {
...
method _pixel_rgb_component_value ($value = undef) {
    ...
    if ( defined $value ) {                          # setter
        ...
        # can only be 0-255
        croak "Value '$value' is not a positive integer"
            if not $self->int_pos($value);
        croak "Value '$value' not in range 0-255" if $value > 255;
        ...
        if ( $self->_has_pixel_property('rgb_component_index]') ) {
            ...
            # initialise
            $self->_set_pixel_properties(
                rgb_component_value => $color[$index] );
...
method _pixel_rgb_component_increment () {
    return ( $self->_pixel_rgb_component_value <= 127 ) ? 6 : -6;
}    #                                                                 }}}1
