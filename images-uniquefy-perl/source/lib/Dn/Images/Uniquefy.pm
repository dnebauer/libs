...
method uniquefy_images () {
    ...
    for my $file (@files) {
        ...
        my $image
            = Dn::Images::Uniquefy::ModifyImage->new( filepath => $file );
...
# prints: error message on failure
# return: Dn::Images::Uniquefy::ModifyImage object
method _new_image ($file) {
    return Dn::Images::Uniquefy::ModifyImage->new( filepath => $file );
}
...
method _is_unique ($file) {
    ...
    confess 'No filepath provided' if not $file;
...
method _set_next_pixel ($image) {
    ...
    if ( not $image ) { confess 'No image provided'; }
...
Dn::Images::Uniquefy::PixelsProcessed, Role::Utils::Dn, English, experimental,
